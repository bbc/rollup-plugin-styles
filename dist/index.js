...
var resolveAsync$1__default = _interopDefault(resolveAsync$1);
var crypto = require('crypto');
var cosmiconfig = require('cosmiconfig');
var valueParser = _interopDefault(require('postcss-value-parser'));
var mimeTypes = require('mime-types');
var modulesValues = _interopDefault(require('postcss-modules-values'));
var localByDefault = _interopDefault(require('postcss-modules-local-by-default'));
var extractImports = _interopDefault(require('postcss-modules-extract-imports'));
var modulesScope = _interopDefault(require('postcss-modules-scope'));
...
class MapModifier {
  ...
  relative(dir = process.cwd()) {
    return this.modifySources(source => {
      if (source === "<no source>") return source;
      if (isAbsolutePath(source)) return relativePath(dir, source);else return normalizePath(source);
    });
  }
...
var resolveAsync = (async (id, options = {}) => new Promise((resolve, reject) => {
  resolveAsync$1__default(id, options, (err, res) => err ? reject(err) : resolve(res));
}));
...
const plugin = postcss.plugin(name, (options = {}) => async (css, res) => {
  ...
  if (!((_css$source = css.source) === null || _css$source === void 0 ? void 0 : _css$source.input.file)) return;
  ...
  for await (const {
    ...
    try {
      ...
      if (!imported.root) importRule.remove();else importRule.replaceWith(imported.root);
    } catch {
...
const plugin$1 = postcss.plugin(name$1, (options = {}) => async (css, res) => {
  var _css$source, _options$inline, _options$publicPath, _options$assetDir, _options$resolve, _options$alias, _options$hash, _css$source$input$map;
  ...
  css.walkDecls(decl => {
    ...
    walkUrls(parsed, (url, node) => {
      ...
      if (((_decl$source = decl.source) === null || _decl$source === void 0 ? void 0 : _decl$source.input.file) && imported.includes(decl.source.input.file)) basedirs.add(path.dirname(decl.source.input.file)); // Use SourceMap

      if ((_decl$source2 = decl.source) === null || _decl$source2 === void 0 ? void 0 : _decl$source2.start) {
...
const loader = {
  ...
  async process({
    ...
    if (options.minimize) plugins.push(cssnano(typeof options.minimize === "object" ? options.minimize : {})); // Avoid PostCSS warning
    ...
    if (!options.extract && this.sourceMap) res.css += mm(map).modify(map => void delete map.file).relative().toCommentData();
    ...
    if (options.inject) {
      if (typeof options.inject === "function") {
        ...
        if (!injectorId) {
          ...
          }).then(normalizePath).then(JSON.stringify);
        }
...
const importer = (url, importer, done) => {
  ...
  resolveAsync(partialUrl, options).then(finalize).catch(() => {
...
const loader$3 = {
  ...
  async process({
    ...
    if (((_style$sourcemap = style.sourcemap) === null || _style$sourcemap === void 0 ? void 0 : _style$sourcemap.sources) && !style.sourcemap.sourcesContent) {
...
var index = ((options = {}) => {
  ...
  const plugin = {
    ...
    async transform(code, id) {
      ...
      return {
        code: res.code,
        map: (_ref = sourceMap ? res.map : undefined) !== null && _ref !== void 0 ? _ref : {
          mappings: ""
        },
    ...
    augmentChunkHash(chunk) {
      ...
      const hashable = extracted.filter(e => ids.includes(e.id)).sort((a, b) => ids.lastIndexOf(a.id) - ids.lastIndexOf(b.id)).map(e => {
        const {
          base,
          dir
        } = path.parse(e.id);
    ...
    async generateBundle(opts, bundle) {
        ...
        for (const module of Object.keys(chunk.modules)) {
          ...
          while (ids.length > 0) {
            ...
            for (const id of ids) {
              ...
              if (loaders.isSupported(id)) ordered.push(id);else traversed.push(id);
      ...
      for await (const [name, ids] of emittedList) {
        ...
        if (res.map && sourceMap) {
          ...
          const assetDir = opts.assetFileNames ? normalizePath(path.dirname(opts.assetFileNames)) : "assets"; // Default for Rollup v2

          const map = mm(res.map).modify(m => {
          ...
          if (sourceMap.inline) {
            ...
            bundle[fileName].source += map.toCommentFile(mapFileName);
          }
        }
      }
    }
